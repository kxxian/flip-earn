...
const ManageListing = () => {
  ...
  useEffect(() => {
    ...
    const getListingWithId = async () => {
      setIsEditing(true);
      setLoadingListing(true);
      const listing = userListings.find((listing) => listing.id === id) ?? null;
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex justify-end gap-3 text-sm">
            <button
              type="button"
              className="px-6 y-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
              onClick={() => navigate(-1)}
            >
              Cancel
            </button>
